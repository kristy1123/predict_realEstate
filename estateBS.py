...
df=pd.read_csv('./estateBS.csv', header=0, names=['years','Busan']) #헤더가 0이면 첫번째 열을 이름으로 읽음
...
plot_acf(df.Busan)#레퍼런스코드

#차분
f=plt.figure()
ax1=f.add_subplot(121)   #왜 121?
ax1.set_title('1st dif')
dif_1=ax1.plot(df.Busan.diff())

ax2=f.add_subplot(122) #왜 122?
plot_acf(df.Busan.diff().dropna(), ax=ax2)
plt.show()
...
# ax3.plot(df.Busan.diff().diff())

# ax4=f2.add_subplot(122) #왜 122?
# plot_acf(df.Busan.diff().diff().dropna(), ax=ax4) #dropna로 값이 비어있는 곳 삭제.
# plt.show()
...
# ax5.plot(df.Busan.diff().diff().diff())

# ax6=f3.add_subplot(122) #왜 122?
# plot_acf(df.Busan.diff().diff().diff().dropna(), ax=ax6)
# plt.show()
...
#
result_dif1=adfuller(df.Busan.diff().dropna())
print('차분 1번한 p-value : ', result_dif1[1])

result_dif2=adfuller(df.Busan.diff().diff().dropna())
print('차분 2번한 p-value : ', result_dif2[1])

result_dif3=adfuller(df.Busan.diff().diff().diff().dropna())
...
arima_model=ARIMA(df.Busan, order=(1,2,2))
