...
Ulsan['years'] = pd.to_datetime(Ulsan['years']) #str to pandas Timestamp
Ulsan.index = Ulsan['years']
Ulsan.set_index('years',inplace=True) #index로 변환
...
result = seasonal_decompose(Ulsan, model='additive')
...
def kpss_test(Ulsan, **kw):
    statistic, p_value, n_lags, critical_values = kpss(Ulsan, **kw)
    ...
    for key, value in critical_values.items():
        print(f'   {key} : {value}')
    print(f'Result: The Ulsan is {"not " if p_value < 0.05 else ""} stationary')
...
diff_1=Ulsan.diff(periods=1).iloc[1:] #차분1
