...
df=pd.read_csv('./EstateSL.csv', header=0, names=['years','Seoul']) #헤더가 0이면 첫번째 열을 이름으로 읽음
...
plot_acf(df.Seoul) #레퍼런스코드

# 차분
f=plt.figure()
ax1=f.add_subplot(121)   #왜 121?
ax1.set_title('1st dif')
dif_1=ax1.plot(df.Seoul.diff())

ax2=f.add_subplot(122) #왜 122?
plot_acf(df.Seoul.diff().dropna(), ax=ax2)
plt.show()
...
# ax3.plot(df.Seoul.diff().diff())
#
# ax4=f2.add_subplot(122) #왜 122?
# plot_acf(df.Seoul.diff().diff().dropna(), ax=ax4)
# plt.show()
#
# #차분 3번째
# f3=plt.figure()
...
# ax5.plot(df.Seoul.diff().diff().diff())
#
# ax6=f3.add_subplot(122) #왜 122?
# plot_acf(df.Seoul.diff().diff().diff().dropna(), ax=ax6)
# plt.show()
...
# ax7.plot(df.Seoul.diff().diff().diff().diff().diff().diff().diff().diff().diff().diff())
#
# ax8=f4.add_subplot(122) #왜 122?
# plot_acf(df.Seoul.diff().diff().diff().diff().diff().diff().diff().diff().diff().diff().dropna(), ax=ax8)
# plt.show()
#===> 이거는 줄일 수 있지 않을까?? 한번 알아볼 것

from statsmodels.tsa.stattools import adfuller
# result=adfuller(df.Seoul.dropna())
# print('p-value : ', result[1])
#
result_dif1=adfuller(df.Seoul.diff().dropna())
print('차분 1번한 p-value : ', result_dif1[1])

result_dif2=adfuller(df.Seoul.diff().diff().dropna())
print('차분 2번한 p-value : ', result_dif2[1])

result_dif3=adfuller(df.Seoul.diff().diff().diff().dropna())
...
#
# ax2=f.add_subplot(122) #왜 122?
# plot_pacf(df.Seoul.diff().dropna(), ax=ax2)
# plt.show()
...
from statsmodels.tsa.arima.model import ARIMA   # arima.model을 사용해야함 arima_model=ARIMA(df.price, order=(2,2,1))
arima_model=ARIMA(df.Seoul, order=(1,2,1))
