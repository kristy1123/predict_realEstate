...
Daejeon['years'] = pd.to_datetime(Daejeon['years']) #str to pandas Timestamp
Daejeon.index = Daejeon['years']
Daejeon.set_index('years',inplace=True) #index로 변환
...
result = seasonal_decompose(Daejeon, model='additive')
...
def kpss_test(Daejeon, **kw):
    statistic, p_value, n_lags, critical_values = kpss(Daejeon, **kw)
    ...
    for key, value in critical_values.items():
        print(f'   {key} : {value}')
    print(f'Result: The Daejeon is {"not " if p_value < 0.05 else ""} stationary')
...
diff_1=Daejeon.diff(periods=1).iloc[1:] #차분1
