...
df=pd.read_csv('./estateGG.csv', header=0, names=['years','Gyeonggi']) #헤더가 0이면 첫번째 열을 이름으로 읽음
...
plot_acf(df.Gyeonggi)#레퍼런스코드

# #차분
# f=plt.figure()
# ax1=f.add_subplot(121)   #왜 121?
# ax1.set_title('1st dif')
# dif_1=ax1.plot(df.Gyeonggi.diff())
#
# ax2=f.add_subplot(122) #왜 122?
# plot_acf(df.Gyeonggi.diff().dropna(), ax=ax2)
# plt.show()
...
ax3.plot(df.Gyeonggi.diff().diff())

ax4=f2.add_subplot(122) #왜 122?
plot_acf(df.Gyeonggi.diff().diff().dropna(), ax=ax4)
plt.show()
...
# ax5.plot(df.Gyeonggi.diff().diff().diff())
#
# ax6=f3.add_subplot(122) #왜 122?
# plot_acf(df.Gyeonggi.diff().diff().diff().dropna(), ax=ax6)
# plt.show()
...
# ax7.plot(df.Gyeonggi.diff().diff().diff().diff().diff().diff().diff().diff().diff().diff())
#
# ax8=f4.add_subplot(122) #왜 122?
# plot_acf(df.Gyeonggi.diff().diff().diff().diff().diff().diff().diff().diff().diff().diff().dropna(), ax=ax8)
# plt.show()
#===> 이거는 줄일 수 있지 않을까?? 한번 알아볼 것


from statsmodels.tsa.stattools import adfuller
result=adfuller(df.Gyeonggi.dropna())
print('p-value : ', result[1])

result_dif1=adfuller(df.Gyeonggi.diff().dropna())
print('차분 1번한 p-value : ', result_dif1[1])

result_dif2=adfuller(df.Gyeonggi.diff().diff().dropna())
print('차분 2번한 p-value : ', result_dif2[1])

result_dif3=adfuller(df.Gyeonggi.diff().diff().diff().dropna())
...
#
# ax2=f.add_subplot(122) #왜 122?
# plot_pacf(df.Gyeonggi.diff().dropna(), ax=ax2)
# plt.show()

#ARIMA(p,d,q)
#p= pacf에서 가장 안정적이었던숫자 선택. 2번이었음. p=2
#d= p값이 임계를 넘기 전 값으로 설정. 2번이었음. d=2
#q= acf에서 가장 안정적이었던 숫자 선택. 1과 2가 비슷했음. 일단 1을 선택하고 후에 값을 바꿔볼 것. q=1
...
arima_model=ARIMA(df.Gyeonggi, order=(1,2,2))
